select
    ...
    mcc2.name as Curso,
    case
        when carga_horaria.value is null
        or carga_horaria.value = '' then 'Não informado'
        else carga_horaria.value
    ...
    case
        ...
        and gg.finalgrade < 7
        and ((mq.attempts = count(distinct mqa.id))) then 'Reprovado'
        when count(mcmc.id) > 0
        and (
            gg.finalgrade is null
            or (
                gg.finalgrade > 0
                and count(distinct mqa.id) < mq.attempts
            )
        ) then 'Em andamento'
    end as processo
from
    ...
    left join mdl_course_modules_completion mcmc on mcm.id = mcmc.coursemoduleid
    ...
    and gg.userid = u.id
    inner join mdl_course_categories cc on cc."path" ~ '/752/'
    ...
    inner join mdl_quiz mq on mq.course = c.id
    left join mdl_quiz_attempts mqa on mqa.quiz = mq.id
    and mqa.userid = u.id
    left join mdl_customfield_data carga_horaria on carga_horaria.fieldid = 9
    and carga_horaria.instanceid = c.id
group by
    curso,
    disciplina,
    c.id,
